...
function renderShop() {
    ...
    const productHTML = allProducts.map(p => {
        ...
        <div class="pro" style="${p.stock <= 0 ? 'opacity:0.6;' : ''}">
            ...
            <img src="${p.img}" alt="" onclick="window.location.href='sproduct.html?id=${p.id}'">
...
if (addProductForm) {
    ...
    sellerEmail: currentSeller?.email || "unknown"
};
...
function renderBestSellers() {
    ...
    <div class="pro">
        <img src="${p.img}" alt="${p.name}" onclick="window.location.href='sproduct.html?id=${p.id}'">
